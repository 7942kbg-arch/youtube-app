...
st.set_page_config(page_title="Hiview Visual", layout="wide")
...
def format_number(num):
    ...
    if num >= 10000: return f"{num//10000}만"
    return f"{num}"
    ...
    try:
        ...
        for item in videos['items']:
            ...
            if "H" not in content['duration']:
                if int(duration_str.split(':')[0]) < 1: is_shorts = "🩳"
                elif duration_mode == "숏츠 (<4분)": is_shorts = "🎞️"
            ...
            channel_display = f"{snip['channelTitle']} ({subs_fmt}명)"

            data.append({
                "썸네일": snip['thumbnails']['high']['url'],
                "길이": duration_str,
                "제목": snip['title'],
                "채널정보": channel_display,
                "조회수": views,
...
keyword = c1.text_input("검색어 입력", placeholder="예: 뷰티 꿀팁", key="keyword_input")
...
if run_btn and keyword:
    ...
    with st.spinner("데이터 수집 중..."):
        ...
        if result:
            ...
            st.download_button("💾 엑셀 저장", csv, f"{keyword}_data.csv", "text/csv")

            st.dataframe(
                df,
                column_config={
                    "썸네일": st.column_config.ImageColumn("썸네일", width="large"),
                    "길이": st.column_config.TextColumn("시간", width="small"),
                    "제목": st.column_config.TextColumn("제목", width="large"),
                    "채널정보": st.column_config.TextColumn("채널 (구독자)", width="medium"),
                    "조회수": st.column_config.NumberColumn("조회수", format="%d회"),
                    "참여율": st.column_config.NumberColumn("참여율", format="%.2f%%"),
                    "실적": st.column_config.TextColumn("등급", width="small"),
                    "공헌": st.column_config.TextColumn("기여", width="small"),
                    "링크": st.column_config.LinkColumn("링크", display_text="이동"),
                    "좋아요": st.column_config.NumberColumn("좋아요", format="%d"),
                    "구독자(정렬용)": st.column_config.NumberColumn("구독자수(정렬용)", format="%d명"),
                    "채널키워드": st.column_config.TextColumn("채널키워드"),
                },
                column_order=[
                    "썸네일", "길이", "제목", "채널정보", 
                    "조회수", "참여율", "실적", "공헌", 
